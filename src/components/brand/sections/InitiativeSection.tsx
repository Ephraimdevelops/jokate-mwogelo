...
export function InitiativeSection({ name, desc, target, action, imageSide, imageSrc }: InitiativeProps) {
  return (
        ...
        <div className="w-full lg:w-1/2 flex flex-col justify-center px-6 lg:px-24 py-20 lg:py-32 bg-brand-white">
          ...
          >
            ...
            <motion.div variants={fadeUp} className="grid grid-cols-1 sm:grid-cols-2 gap-8 mb-16 pt-8 border-t border-brand-border">
               <div>
                 <span className="text-[10px] font-sans uppercase tracking-widest text-brand-gold block mb-2">Impact Demographic</span>
                 <span className="text-body font-sans text-brand-black opacity-90">{target}</span>
               </div>
               <div>
                 <span className="text-[10px] font-sans uppercase tracking-widest text-brand-gold block mb-2">Engagement Protocol</span>
                 <span className="text-body font-sans text-brand-black opacity-90">{action}</span>
               </div>
            </motion.div>

            <motion.div variants={fadeUp}>
               <Link href="/contact" className="group inline-flex items-center gap-4 bg-brand-black text-brand-white px-10 py-5 text-[12px] uppercase tracking-[0.15em] font-sans hover:bg-brand-gold transition-all duration-500 shadow-sm hover:shadow-xl">
                  Deploy to Region <span className="text-brand-gold text-[16px] leading-[0] transition-transform duration-500 group-hover:translate-x-2">→</span>
               </Link>
            </motion.div>
          </motion.div>
        </div>
